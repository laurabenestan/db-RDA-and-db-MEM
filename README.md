...
* **Create an environmental matrix (PCA accept missing value)
```{r}
Env_pca=PCA(Env, scale.unit=FALSE, ncp=10)
Env_pca$eig
```

* **Keep the eigenvalue > 1 and indicate the number of eigenvalue to keep for the PCA on environmental data
```{r}
PCeigen_1=3
ind=Env_pca$ind
...
### Perform a PCA on the genomic data
```{r}
Geno_pca=prcomp(Geno)
summary(Geno_pca)
```

### Keep number of axes which explained percent of variance that explained more than 5% (or the percent of cumulative variance explained more than 90%). Here we kept the percent of variance that explained more than 5%. 
```{r}
Geno_rda=Geno_pca$x[,1:9]
```

### Do a RDA with the 3 axes of the environmental PCA (PCA factors)
```{r}
...
### Create a graphic with RDA1 and RDA2
```{r}
axes=c(1,2);
R2 = c("9.4%","8.3%")
